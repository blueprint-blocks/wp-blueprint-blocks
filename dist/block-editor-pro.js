(function (hooks) {
	...
	function convertDomNodesToTokenizedString() {
		...
		var _string = string.replaceAll(
			/<span data-type=\"token\">\s?(.*?)<\/span>/g,
			function (match, p1) {
				return "{{ ".concat(p1.trim(), " }}");
			},
		);
		_string = _string.replaceAll(
			/<span data-type=\"cursor-start\">(.*?)<\/span>/g,
			function (match, p1) {
				return p1;
			},
		);
		_string = _string.replaceAll(
			/<span data-type=\"cursor-end\"> <\/span>/g,
			function (match) {
				return "";
			},
		);
		_string = _string.replaceAll(
			/<span data-type=\"cursor-end\">\s(.*?)<\/span>/g,
			function (match, p1) {
				return p1;
			},
		);
	...
	function convertTokenizedStringToDomNodes() {
		...
		_string = _string.replaceAll(/\{\{(.*?)\}\}/g, function (match, p1) {
			return '<span data-type="token"> '.concat(
				p1.trim(),
				'</span><span data-type="cursor-end"> </span>',
			);
		});
		return _string;
	}
