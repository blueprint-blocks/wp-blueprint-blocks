function tokenizeObject(object, ancestry = "") {
	...
	if (
		tokenizedObject &&
		typeof tokenizedObject === "object" &&
		tokenizedObject.constructor === Object
	) {
		for (const [key, value] of Object.entries(tokenizedObject)) {
			if (typeof value === "object") {
				tokenizedObject[key] = tokenizeObject(
					value,
					(ancestry && `${ancestry}.${key}`) || key,
				);
			} else {
				tokenizedObject[key] =
					`{{ ${(ancestry && ancestry + ".") || ""}${key}.${typeof value} }}`;
			}
		}
	}
