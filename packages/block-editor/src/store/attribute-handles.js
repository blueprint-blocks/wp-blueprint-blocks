...
const slice = createSlice( {
	...
	reducers: {
		setPosition( state, action ) {
			const { id, name, componentId, context = 'to', x, y } = action.payload
		...
		removePosition( state, action ) {
			...
			state.allHandles = Object.fromEntries(Object.entries( state.allHandles ).filter( ( [ key, value ] ) => (
				key !== id
			) ) )
			...
			if ( state.handlesByName?.[ name ]?.from === id ) {
				...
				state.handlesByName = {
					...
					[ name ]: {
						from: state.handlesByName?.[ name ]?.from || null,
						to: ( state.handlesByName?.[ name ]?.to || [] ).filter( ( to ) => (
							to !== id
						) ),
					},
				}
