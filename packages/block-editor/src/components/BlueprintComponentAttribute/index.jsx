...
import { getBlockComponent, setComponentAttribute, unsetComponentAttribute } from '../../store/block-blueprint'
...
function BlueprintComponentAttribute( {
	...
	const { currentFocus } = useSelector( ( state ) => (
		state.editor
	) )
	...
	const onChangeAttributeName = useCallback( ( newAttributeName ) => {
		let newAttributeValue = structuredClone( _attributeValue )
	...
	const onChangeAttributeValue = useCallback( ( newAttributeValue ) => {
		dispatch( setComponentAttribute( {
			clientId,
			attribute: _attributeName,
			value: newAttributeValue,
	...
	return (
		<div ref={ ref } className={ classNames( 'BlueprintComponentAttribute', `is-${ attributeType }`, {
			...
			<span className="BlueprintComponentAttribute-seperator">
				{ '=' }
			</span>

			{ _attributeName === 'style' && (
				<>
					<span className="BlueprintComponentAttribute-open">
						{ '"' }
					</span>
					...
					<span className="BlueprintComponentAttribute-close">
						{ '"' }
					</span>
				</>
			) }
			{ _attributeName !== 'style' && attributeType === 'object' && (
				<>
					<span className="BlueprintComponentAttribute-open">
						{ '{' }
					</span>
			...
			{ attributeType !== 'object' && (
				<>
					<span className="BlueprintComponentAttribute-quote">
						{ '"' }
					</span>
					<span className="BlueprintComponentAttribute-value">
						<EditableString
