...
function BlueprintSidebar( {
	...
	const isDragging = useSelector( ( state ) => (
		state.blockBlueprint.isDragging
	) )
	...
	useLayoutEffect( () => {
		contextPanelRef.current.classList.toggle( 'is-active', !!currentFocus?.context )
		componentsPanelRef.current.classList.toggle( 'is-active', !currentFocus?.context )
	}, [ currentFocus ] )
