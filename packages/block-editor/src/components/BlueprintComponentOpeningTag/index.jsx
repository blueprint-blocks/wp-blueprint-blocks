...
const BlueprintComponentOpeningTag = forwardRef( ( {
	...
	return (
		<div className="BlueprintComponent-open" ref={ ref }>
			...
			<div className={ `BlueprintComponent-markup ${ isMultiLine && 'is-multi-line' || '' }` }>
				<div className="BlueprintComponent-line">
					<span>{ `<` }</span>
					<span className="BlueprintComponent-tagName">{ tagName }</span>
				</div>
				{ componentAttributes.map( ( [ name, value ], index ) => (
					...
					>
						{ ( !isMultiLine && index === componentAttributes.length - 1 ) && (
							<>
								{ !allowsChildren && (
									<span>{ `/` }</span>
								) }
