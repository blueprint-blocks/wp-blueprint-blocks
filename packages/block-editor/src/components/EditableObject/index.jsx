...
const EditableObject = memo( ( {
	...
	const onChangePropertyName = useCallback( ( propertyName, newPropertyName ) => {
	...
	const onChangePropertyValue = useCallback( ( propertyName, newPropertyValue ) => {
		let newValue = { ..._value }
		newValue[ propertyName ] = newPropertyValue
		onChange && onChange( newValue )
	}, [ _value ] )

	const onDeleteProperty = useCallback( ( propertyName ) => {
	...
	return (
		...
		>
			<div className="EditableObject-properties">
				{ Object.entries( value ).map( ( [ propertyName, propertyValue ], index ) => (
					<div className="EditableObject-property">
						<span className="EditableObject-value">
							<EditableString
								...
								onBlur={ _onBlur }
								onChange={ ( newPropertyName ) => onChangePropertyName( propertyName, newPropertyName ) }
								onFocus={ _onFocus }
								onDelete={ () => onDeleteProperty( propertyName ) }
							/>
						</span>
						...
						<span className="EditableObject-value">
							<EditableString
								...
								onBlur={ _onBlur }
								onChange={ ( newPropertyValue ) => onChangePropertyValue( propertyName, newPropertyValue ) }
								onFocus={ _onFocus }
							/>
						</span>
						{ ( propertySeperator !== ',' || index < Object.entries( _value ).length - 1 ) && (
							<span className="EditableObject-seperator">
								{ propertySeperator }
							</span>
						) }
